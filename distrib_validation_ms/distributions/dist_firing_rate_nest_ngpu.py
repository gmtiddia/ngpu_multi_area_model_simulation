...
# matrix in which we store the distributions for every population
...
for i_run in range(10):
    for i in range(254):
        fn1 = path+'mam_nest3_ms_validation/data' + str(i_run) + '/firing_rate_' + \
              str(i) + '.dat'
        fn2 = path+'mam_nestgpu_ms_validation/data' + str(i_run) + '/firing_rate_' \
              + str(i) + '.dat'
        ...
        # some files can be empty (especially for correlation and cv isi that requires certain conditions to compute the distributions)
        if(len(y1)>0 and len(y2)>0):
            ...
            # here we average the results of the distributions to obtain a single smooth distribution for every population
            eps = 1.0e-3
...
# plot that saves all the distributions in PDFs to be merged to obtain the full list of pop distributions
ticks=20
legenda=24
label=22
titolo=30
...
for j in range(32):
    if j<31:
        ...
        for i in range(8):
            ...
            if i+1>6:
                plt.xlabel("Fring rate [spikes/s]", fontsize=label)
        ...
        for i in range(6):
            ...
            plt.ylabel("p", fontsize=label)
            if i+1>4:
                plt.xlabel("Firing rate [spikes/s]", fontsize=label)
            plt.tick_params(labelsize=ticks)
            plt.legend(fontsize=legenda)
            plt.grid()
        figure = plt.gcf()  # get current figure
