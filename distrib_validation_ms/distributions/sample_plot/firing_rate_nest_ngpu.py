...
y1std = np.zeros((254, 300))   
y2std = np.zeros((254, 300))   
y3std = np.zeros((254, 300))   

path='../../../data/'

for i in range(8):
      y1vals = np.zeros((300,10))
      y2vals = np.zeros((300,10))
      y3vals = np.zeros((300,10))
      for i_run in range(10):
            fn1 = path+'mam_original_ms3/data' + str(i_run) + '/firing_rate_' + \
            str(i) + '.dat'
            fn3 = path+'mam_original_ms4/data' + str(i_run) + '/firing_rate_' + \
            str(i) + '.dat'
            fn2 = path+'mam_ngpu_ms/data' + str(i_run) + '/firing_rate_ngpu1_' \
            + str(i) + '.dat'
...
for i in range(8):
    ...
    #plt.title(tit[i%2], fontsize=ticks, fontweight='bold', loc='left')
    plt.title(poplabel[i], fontsize=ticks, loc='center')
    ...
    plt.plot(xnew, y2mean[i], '--', color="r", label="NEST GPU")
    plt.fill_between(xnew, y2mean[i]-y2std[i], y2mean[i]+y2std[i], color="red", alpha = 0.25)
    plt.xlabel("rate [spikes/s]", fontsize=label)
    plt.tick_params(labelsize=ticks)
    plt.legend(fontsize=legenda)
    plt.grid()
    #plt.savefig("plot_dist/firing_rate/band_"+popname[i]+"2NEST", dpi=300)
